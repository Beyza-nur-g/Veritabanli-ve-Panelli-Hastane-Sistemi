...
<form action="hasta_ekle.php" method="post">
  ...
  <button type="submit" class="btn">Hasta Ekle</button>
    ...
    <script>
    document.getElementById("hastaEkleForm").addEventListener("submit", function(event) {
            event.preventDefault(); // Formun submit işlemini engelle
            
            // Form verilerini al
            var formData = new FormData(this);
            
            // AJAX kullanarak formu post et
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "hasta_ekle.php", true);
            xhr.onload = function() {
                if (xhr.status === 200) {
                    // Hasta ekleme başarılıysa yönlendir
                    window.location = "randevu.php";
                } else {
                    // Hata durumunda kullanıcıya bilgi ver
                    console.error("Hasta eklenirken bir hata oluştu.");
                }
            };
            xhr.send(formData);
        });
